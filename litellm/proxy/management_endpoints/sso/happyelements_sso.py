...
class HappyElementsSSO:
    ...
    def generate_login_url(
        ...
        Args:
            ...
            callback_url_override: Override the default callback URL (e.g. to embed
                                   a CLI session key for the CLI SSO flow).

        Returns:
            Complete SSO login URL
        """
        timestamp = int(time.time())
        effective_callback = callback_url_override or self.callback_url

        verbose_proxy_logger.debug(
            f"Generating HappyElements SSO login URL: timestamp={timestamp}, ip={client_ip}, callback={effective_callback}"
        )
    ...
    def process_callback(
        ...
        verbose_proxy_logger.debug(f"Decrypted RspToken: username={username}, ip={client_ip}, timestamp={timestamp}")
        ...
        if time_diff > 300:  # 5 minutes
            verbose_proxy_logger.warning(f"SSO callback timestamp is {time_diff} seconds old (threshold: 300s)")
        ...
        if response_extra:
            try:
                ...
                verbose_proxy_logger.error(
                    f"❌ Failed to decrypt ResponseExtra: {type(e).__name__}: {e}. Falling back to RspToken data only."
                )
        else:
            verbose_proxy_logger.warning(
