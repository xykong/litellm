...
async def _setup_team_mappings() -> Optional["TeamMappings"]:
    ...
    try:
        ...
        sso_db_record = await prisma_client.db.litellm_ssoconfig.find_unique(where={"id": "sso_config"})
...
async def _setup_role_mappings() -> Optional["RoleMappings"]:
    ...
    generic_role_mappoings_default_role = os.getenv("GENERIC_ROLE_MAPPINGS_DEFAULT_ROLE", None)
...
async def cli_sso_callback(
    ...
    parsed_openid_result = SSOAuthenticationHandler._get_user_email_and_id_from_result(result=result_non_none)
    ...
    try:
        ...
        if session_data:
            ...
            if session_data.get("key") and session_data.get("status") == "ready":
                ...
                user_api_key_cache.delete_cache(key=cache_key)
                verbose_proxy_logger.info(
                    f"CLI poll: returning pre-resolved virtual key for user={pre_resolved_user_id}"
                )
                return {
                    "status": "ready",
            ...
            jwt_token = ExperimentalUIJWTToken.get_cli_jwt_auth_token(user_info=user_info, team_id=team_id)
...
class SSOAuthenticationHandler:
    ...
    async def upsert_sso_user(
        ...
        try:
            if user_info is not None:
                ...
                await prisma_client.db.litellm_usertable.update_many(where={"user_id": user_id}, data=update_data)
            else:
    ...
    async def create_litellm_team_from_sso_group(
        ...
        try:
            team_obj = await prisma_client.db.litellm_teamtable.find_first(where={"team_id": litellm_team_id})
