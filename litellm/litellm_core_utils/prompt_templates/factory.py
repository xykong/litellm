...
def sanitize_anthropic_native_messages_for_tool_calling(
    ...
    while message_index < len(sanitized_messages):
        ...
        if reorder_needed:
            current_message["content"] = non_tool_use_blocks + tool_use_blocks
        ...
        if isinstance(next_content, list):
            ...
            normalized_next_content = [{"type": "text", "text": next_content}] if next_content else []
        elif next_content is None:
            normalized_next_content = []
        else:
            normalized_next_content = [next_content]

        next_message["content"] = missing_tool_result_blocks + normalized_next_content
        message_index += 1
