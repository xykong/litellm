...
client = openai.OpenAI(
	api_key="${b||"YOUR_LITELLM_API_KEY"}",
	base_url="${I}"
)`;switch(_){case o.CHAT:{let e=Object.keys(T).length>0,a="";if(e){let e=JSON.stringify({metadata:T},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let i=y.length>0?y:[{role:"user",content:x}];t=`
import base64
...
`;break}case o.RESPONSES:{let e=Object.keys(T).length>0,a="";if(e){let e=JSON.stringify({metadata:T},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let i=y.length>0?y:[{role:"user",content:x}];t=`
import base64
