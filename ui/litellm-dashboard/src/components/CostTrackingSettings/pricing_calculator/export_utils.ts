...
export const exportToPDF = (result: CostEstimateResponse): void => {
  ...
  const html = `
    ...
    <body>
      <h1>🦁 Animal Gateway Cost Estimate Report</h1>
      ...
      <div class="footer">
        <p>Generated by Animal Gateway Pricing Calculator on ${new Date().toLocaleString()}</p>
      </div>
    </body>
    </html>
  `;
...
export const exportToCSV = (result: CostEstimateResponse): void => {
  const rows = [
    ["🦁 Animal Gateway Cost Estimate Report"],
    [""],
    ["Configuration"],
    ["Model", result.model],
