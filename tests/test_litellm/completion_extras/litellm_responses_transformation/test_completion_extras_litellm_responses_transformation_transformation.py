...
def test_commentary_phase_message_is_skipped_in_convert_response_output_to_choices():
    """
    Regression test: Azure gpt-5.4 (and newer Responses API models) may return a
    `phase: "commentary"` message before the actual tool_calls.  This message
    represents intermediate model planning/reasoning and must NOT be surfaced as a
    choice.  If it were, clients (e.g. OpenCode) would see text content in
    choices[0] instead of the tool_calls, breaking the agent loop entirely.

    Expected behaviour after the fix:
    - The commentary message is silently skipped.
    - The tool_call is promoted to choices[0] with finish_reason="tool_calls".
    """
    from litellm.completion_extras.litellm_responses_transformation.transformation import (
        LiteLLMResponsesTransformationHandler,
    )
    ...
    # Build a synthetic Responses-API output list that mirrors what gpt-5.4 returns:
    #   1. type=message, phase="commentary"  <- must be skipped
    #   2. type=function_call                <- must become choices[0]
    commentary_msg = ResponseOutputMessage(
        id="msg_commentary",
        type="message",
        role="assistant",
        content=[
    ...
    object.__setattr__(commentary_msg, "phase", "commentary")
    ...
    output_items = [commentary_msg, tool_call]
...
def test_non_commentary_phase_message_is_not_skipped():
    """
    Verify that messages with phase=None or phase='final_answer' are still
    emitted as choices (i.e. the fix does not over-filter).
    """
    from litellm.completion_extras.litellm_responses_transformation.transformation import (
        LiteLLMResponsesTransformationHandler,
    )
    ...
    final_msg = ResponseOutputMessage(
        id="msg_final",
        type="message",
        role="assistant",
        content=[
